...
export const chunksToWav = async (base64Chunks: string[], filename: string) => {
  ...
  for (let i = 0; i < completeWavData.length; i += chunkSize) {
    const chunk = completeWavData.slice(
      i,
      Math.min(i + chunkSize, completeWavData.length)
    );
    const binaryString = Array.from(chunk, (byte) =>
...
const ChunkRecorder = () => {
  ...
  return (
    <View className="items-center w-full text-white bg-white">
      {isRecording ? (
        <Text
